...
const TopicDetail = () => {
  ...
  const { 
    ...
    updateNotes, updateSummaryText, uploadDocuments, updateDocumentText, deleteDocument 
  } = useTopic(id);
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
          ...
          {topic.summary && (
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 flex flex-col items-center justify-center text-center gap-4">
              ...
              <div>
                <h3 className="text-lg font-bold text-gray-900 dark:text-white">AI Summary Generated</h3>
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                  Your structured study material is ready to review.
                </p>
              </div>
              <button 
                onClick={() => setShowSummary(true)}
                className="w-full mt-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-medium rounded-lg shadow-sm transition-colors"
              >
                Read Summary
              </button>
            </div>
      ...
      {showSummary && (
        <SummaryModal
          ...
          onSave={async (newSummary) => {
            await updateSummaryText(newSummary);
          }}
        />
      )}
    </div>
